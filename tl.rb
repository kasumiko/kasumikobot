...
nobrakespace = "\u00A0"
	@client.update("#{"@" + username} 永遠ロマンス！#{nobrakespace*rand(20)}", :in_reply_to_status_id => id)
	end

	#ゆゆ式は神アニメ
	if contents =~ /ゆゆ式/
nobrakespace = "\u00A0"
	@client.update("#{"@" + username} ゆゆ式は神アニメ "+nobrakespace*rand(20), :in_reply_to_status_id => id)
	end
	...
	nobrakespace = "\u00A0"
	@client.update("#{"@" + username} ｶﾞｯ#{nobrakespace*rand(5)}", :in_reply_to_status_id => id)
	end
	...
	nobrakespace = "\u00A0"	
	@client.update("#{"@" + username} ん？今なんでもするって言ったよね？#{nobrakespace*rand(5)}", :in_reply_to_status_id => id)
        end
